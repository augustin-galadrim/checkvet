from ._anvil_designer import TEST_AudioManagerEditTemplate
from anvil import *
...
class TEST_AudioManagerEdit(TEST_AudioManagerEditTemplate):
  def __init__(
    ...
    self.transcript = self.report.get("transcript")  # Nouvelle valeur chargée
    ...
    # Ne pas définir le contenu de l'éditeur tout de suite (éviter d'appeler JS avant que le DOM soit prêt)
  ...
  def process_recording(self, audio_blob, **event_args):
    """
    Traite le blob audio : transcrit, génère la mise à jour du rapport et met à jour l'éditeur.
    Un prompt de remplacement fixe est utilisé.
    """
    print("process_recording() appelé avec un blob audio en mode édition.")
    try:
      # Définir un prompt de remplacement fixe.
      self.prompt = (
        "Ceci est un prompt de remplacement pour le traitement audio en secrétariat."
      )
      # Transcrire l'audio.
      transcription = anvil.server.call("process_audio_whisper", audio_blob)
      # Générer le contenu du rapport à partir de la transcription et du prompt.
      report_content = anvil.server.call("generate_report", self.prompt, transcription)
      # Formater le contenu du rapport en texte riche.
  ...
  def relaunch_ai(self, **event_args):
    ...
    try:
      # Utiliser un prompt fixe et le transcript pour générer le contenu du rapport.
      default_prompt = (
        "Ceci est un prompt de remplacement pour le traitement audio en secrétariat."
      )
      report_content = anvil.server.call(
        "generate_report", default_prompt, self.transcript
      )
      report_final = anvil.server.call("format_report", report_content)
      self.editor_content = report_final
  ...
  def update_report(self, ignored_file_name, content_json, images, **event_args):
    """
    Appelé lorsque l'utilisateur clique sur "Archiver". Cette méthode met à jour l'enregistrement
    existant du rapport en appelant la fonction serveur write_report avec les paramètres dans l'ordre attendu :
