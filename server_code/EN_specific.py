  ...
  for template in all_templates:
    ...
    if owner is not None:
      ...
      if owner_email in [current_user['email'], "en_su@checkvet.com"]:
        print(f"Processing template: {template['template_name']} (Owner: {owner_email})")
        template_dict = {
...
def EN_search_templates(search_input):
  try:
    ...
    en_su_user = next((user for user in app_tables.users.search()
                      if user['email'] == 'en_su@checkvet.com'), None)

    search_term = search_input.strip()

    if not search_term:
      # If no search term, return all templates owned by current user or en_su
      rows = [r for r in app_tables.custom_templates.search()
             if r['owner'] == current_user or
             (en_su_user and r['owner'] == en_su_user)]
    else:
      # Search for term within template_name, limited to records owned by current user or en_su
      rows = [r for r in app_tables.custom_templates.search()
             if search_term.lower() in r['template_name'].lower() and
             (r['owner'] == current_user or
              (en_su_user and r['owner'] == en_su_user))]
...
def EN_pick_template(template_name, header):
  ...
  try:
    ...
    template = next((t for t in app_tables.custom_templates.search(template_name=template_name)
                    if t['owner'] == current_user or
                    (en_su_user and t['owner'] == en_su_user)), None)
