...
def affect_base_template(base_template, user):
  ...
  try:
    ...
    try:
      ...
      print(
        f"[DEBUG] Could not find template_name: {str(e)}. Fallback: '{template_name}'"
      )

    print(f"[DEBUG] Final template_name: '{template_name}'")
    print(
      f"DEBUG: Creating custom template from base template '{template_name}' for user: {user['email']}"
    )
    ...
    existing_templates = list(
      app_tables.custom_templates.search(owner=user, base_template=base_template)
    )
...
def update_base_template(base_template):
  ...
  try:
    print(
      f"Debug: Updating derived templates for base template: {base_template['template_name']}"
    )

    # Find all custom templates linked to this base template
    derived_templates = app_tables.custom_templates.search(base_template=base_template)
    ...
    for template in derived_templates:
      # This function might need revision as prompt fields are removed from custom_templates
      # For now, it is left as is, assuming other logic might still use it.
      pass
